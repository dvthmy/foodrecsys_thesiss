...
class Neo4jService:
    ...
    def get_dish_by_id(self, dish_id: str) -> dict[str, Any] | None:
        ...
        RETURN d.dish_id AS dish_id,
               ...
               d.image_embedding AS embedding,
    ...
    def delete_ingredient(self, name: str) -> dict[str, Any] | None:
        """Delete a pending ingredient without merging.

        The ingredient and all its relationships will be permanently deleted.
        Dishes that contained this ingredient will no longer have the relationship.

        Args:
            name: Ingredient name to delete.
        ...
        query = """
        MATCH (i:Ingredient {name: toLower(trim($name))})
        OPTIONAL MATCH (d:Dish)-[r:CONTAINS]->(i)
        WITH i, count(r) AS affected_dishes
        DETACH DELETE i
        RETURN $name AS deleted_name, affected_dishes
        """
    ...
    def get_pending_ingredients(self) -> list[dict[str, Any]]:
        ...
        RETURN i.name AS name, 
             ...
             i['suggested_merges'] AS suggested_merges,
               ...
               collect(DISTINCT d.name) AS dish_names
    ...
    def create_user(
        ...
        Args:
            ...
            dietary_restrictions: Optional list of dietary restriction names.

        Returns:
            Dictionary with user data including generated user_id.
        """
        import uuid
        ...
        CREATE (u:User {
            user_id: $user_id,
            name: $name,
            normalized_name: toLower(trim($name)),
            age: $age,
            gender: $gender,
            nationality: $nationality,
            created_at: datetime()
        ...
        with self.session() as session:
            ...
            # Link dietary restrictions if provided
            if dietary_restrictions:
                for restriction in dietary_restrictions:
                    self.create_user_restriction_relationship(user_id, restriction)

            return dict(result) if result else {"user_id": user_id, "name": name}
    ...
    def get_user_liked_dishes_with_ingredient_embeddings(
        self,
        user_id: str,
        min_rating: int = 4,
    ) -> list[dict[str, Any]]:
        """Get liked dishes with their ingredient embeddings.
        ...
        Returns:
            List of dictionaries with dish data, ingredients, and ingredient embeddings.
        """
        query = """
        MATCH (u:User {user_id: $user_id})-[r:RATED]->(d:Dish)
    ...
    def get_all_dishes_with_ingredient_embeddings(
        self,
        limit: int | None = None,
    ) -> list[dict[str, Any]]:
        """Get all dishes with their ingredient embeddings.

        Args:
            limit: Optional limit on number of dishes to return.
    ...
    def get_excluded_dishes_with_reasons(
        self,
        dish_ids: list[str],
        restriction_names: list[str],
    ) -> list[dict[str, Any]]:
        """Get dishes excluded by dietary restrictions with reasons.

        Args:
            dish_ids: List of dish IDs to check.
            restriction_names: List of dietary restriction names.

        Returns:
            List of excluded dishes with name and bad ingredients.
        """
        if not restriction_names or not dish_ids:
            return []
        ...
        MATCH (bad_ing:Ingredient)-[:NOT_SUITED_FOR]->(r:DietaryRestriction)
        WHERE r.name IN $restriction_names
        WITH collect(DISTINCT {name: bad_ing.name, restriction: r.name}) AS bad_ing_info

        // Check each dish
        UNWIND $dish_ids AS did
        MATCH (d:Dish {dish_id: did})
        OPTIONAL MATCH (d)-[:CONTAINS]->(i:Ingredient)
        WITH d, collect(DISTINCT i.name) AS ingredients, bad_ing_info
        
        // Find bad ingredients in this dish
        WITH d, ingredients, 
             [ing IN ingredients WHERE ANY(b IN bad_ing_info WHERE b.name = ing) | ing] AS found_bad_ingredients,
             bad_ing_info
        WHERE size(found_bad_ingredients) > 0
        
        // Get restriction info for found bad ingredients
        UNWIND found_bad_ingredients AS bad_ing
        WITH d, bad_ing, 
             [b IN bad_ing_info WHERE b.name = bad_ing | b.restriction][0] AS restriction
        WITH d, collect({ingredient: bad_ing, restriction: restriction}) AS violations
    ...
    #     Returns:
    #         A list of all rating relationships.
    #     """
    #     query = """
    #     MATCH (u:User)-[r:RATED]->(d:Dish)
    #     RETURN u.user_id AS user_id, d.dish_id AS dish_id, r.score AS rating
    #     """
    #     with self.session() as session:
    #         result = session.run(query)
    #         return [dict(record) for record in result]

    # def get_all_users_with_ratings(self) -> list[dict[str, Any]]:
    ...
    #     """
    #     query = """
    #     MATCH (u:User)-[:RATED]->(:Dish)
    #     RETURN DISTINCT u.user_id AS user_id, u.name AS name
    #     """
    #     with self.session() as session:
    #         result = session.run(query)
    #         return [dict(record) for record in result]
    ...
    #     Returns:
    #         A list of ratings, where each rating is a dictionary.
    #     """
    #     query = """
    #     MATCH (u:User {user_id: $user_id})-[r:RATED]->(d:Dish)
    #     RETURN d.dish_id AS dish_id, r.score AS rating
    #     """
    #     with self.session() as session:
    #         result = session.run(query, user_id=user_id)
    #         return [dict(record) for record in result]